import { BaseTSer } from "../../timeseris/BaseTSer"
import { ChartView } from "./ChartView"
import { ChartViewContainer, ClassOfChartViewContainer } from "./ChartViewContainer"
...
export class ChartControl implements IChartControl {
  ...
  /** BASIC_BAR_WIDTH = 6 */
  static readonly PREFERRED_BAR_WIDTHS = Array(
  ...
  private _fixedLeftSideTime = Number.MIN_SAFE_INTEGER;
  private _wBarIdx = 11;
  /** pixels per bar (bar width in pixels) */
  private _wBar = ChartControl.PREFERRED_BAR_WIDTHS[this._wBarIdx]
  ...
  isFixedLeftSideTime() {
    return this._fixedLeftSideTime != Number.MIN_SAFE_INTEGER;
  }
  ...
  growWBar(increment: number) {
    ...
    if (this._wBarIdx < 0) {
      this._wBarIdx = 0
    } else if (this._wBarIdx > ChartControl.PREFERRED_BAR_WIDTHS.length - 1) {
      this._wBarIdx = ChartControl.PREFERRED_BAR_WIDTHS.length - 1
    }

    this.internal_setWBar(ChartControl.PREFERRED_BAR_WIDTHS[this._wBarIdx]);
    this.updateViews();
  }
  ...
  setWBarByNBars(wViewPort: number, nBars: number) {
    ...
    /** decide wBar according to wViewPort. Do not use integer divide here */
    var newWBar = wViewPort * 1.0 / nBars * 1.0

    /** adjust xfactorIdx to nearest */
    if (newWBar < ChartControl.PREFERRED_BAR_WIDTHS[0]) {
      /** avoid too small xfactor */
      newWBar = ChartControl.PREFERRED_BAR_WIDTHS[0]

      this._wBarIdx = 0
    } else if (newWBar > ChartControl.PREFERRED_BAR_WIDTHS[ChartControl.PREFERRED_BAR_WIDTHS.length - 1]) {
      this._wBarIdx = ChartControl.PREFERRED_BAR_WIDTHS.length - 1
    } else {
      let i = 0
      const n = ChartControl.PREFERRED_BAR_WIDTHS.length - 1;
      let breakNow = false
      while (i < n && !breakNow) {
