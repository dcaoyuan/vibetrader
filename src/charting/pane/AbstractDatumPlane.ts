...
import { Scalar } from "../view/scalar/Scalar";
import { ChartPane } from "./ChartPane";
import { DatumPlane } from "./DatumPlane";
import { withVolumePane, WithVolumePane } from "./WithVolumePane";
 ...
 */
export abstract class AbstractDatumPlane /* extends Pane(aview, null) */ implements DatumPlane {
  view: ChartView;
  baseSer: BaseTSer;
  ...
  computeGeometry() {
    ...
    /** adjust if necessary */
    if (this as unknown as ChartPane === this.view.mainChartPane) {
      ...
      (this as unknown as ChartPane) === (this.view as unknown as WithVolumePane).volumeChartPane
    ) {
  ...
  tb(barIndex: number): number {
    return this.view.control.baseSer.timeOfRow(this.rb(barIndex));
  }
